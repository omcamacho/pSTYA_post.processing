...
All_data$num_PTM_Y <- str_count(All_data$Peptide, "Y")
All_data$num_PTM_S <- str_count(All_data$Peptide, "S")
All_data$num_PTM_T <- str_count(All_data$Peptide, "T")
All_data$num_PTM_A <- str_count(All_data$Peptide, "A")

sum(All_data$num_PTM_Y)
sum(All_data$num_PTM_S)
sum(All_data$num_PTM_T)
sum(All_data$num_PTM_A)
...
ggplot(GraphData_count, aes(Match, n)) +
  ...
  xlab("Type of Match") + ylab("Number of PSMs") + facet_wrap(~dataset, ncol = 5) + theme(text = element_text(size = 14))
...
GraphData_count_amino <- GraphData %>% dplyr::count(dataset, PROTEIN_LOC, Amino)

ggplot(GraphData_count_amino, aes(Amino, n)) +
  geom_boxplot() + ggtitle("All Phosphosites") +
  xlab("Amino acid") + ylab("Number of PSMs") + facet_wrap(~dataset, ncol = 5) + theme(text = element_text(size = 14))
...
GraphData_95count_amino <- GraphData_95 %>% dplyr::count(dataset, PROTEIN_LOC, Amino)


ggplot(GraphData_95count_amino, aes(Amino, n)) +
  geom_boxplot() + scale_y_continuous(limits=c(0,50)) + ggtitle("Phosphosites observed up to 50 times and Probability>0.95") +
  xlab("Amino acid") + ylab("Number of PSMs") + facet_wrap(~dataset, ncol = 5) + theme(text = element_text(size = 14))
