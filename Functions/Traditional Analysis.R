TA_Pform <- function(dataf){
  ...
  X1$Vector<-lapply(1:nrow(X1), function(i) c(is.element(Y1$col1[i,]-X1$col1[i,],PTMscore$PTM_positions[i]),is.element(Y1$col2[i,]-X1$col2[i,],
                                              ...
                                              is.element(Y1$col5[i,]-X1$col5[i,],PTMscore$PTM_positions[i]),is.element(Y1$col6[i,]-X1$col6[i,],PTMscore$PTM_positions[i])
                                              ,is.element(Y1$col7[i,]-X1$col7[i,],PTMscore$PTM_positions[i]),is.element(Y1$col8[i,]-X1$col8[i,],PTMscore$PTM_positions[i]),
                                              is.element(Y1$col9[i,]-X1$col9[i,],PTMscore$PTM_positions[i]),is.element(Y1$col10[i,]-X1$col10[i,],
                                                                                                                       PTMscore$PTM_positions[i])
                                              ,is.element(Y1$col11[i,]-X1$col11[i,],PTMscore$PTM_positions[i]),is.element(Y1$col12[i,]-X1$col12[i,],PTMscore$PTM_positions[i]),
                                              is.element(Y1$col13[i,]-X1$col13[i,],PTMscore$PTM_positions[i]),is.element(Y1$col14[i,]-X1$col14[i,],PTMscore$PTM_positions[i])
                                              ,is.element(Y1$col15[i,]-X1$col15[i,],PTMscore$PTM_positions[i]),is.element(Y1$col16[i,]-X1$col16[i,],PTMscore$PTM_positions[i])
                                              ,is.element(Y1$col17[i,]-X1$col17[i,],PTMscore$PTM_positions[i]))) 
  ...
  PTMscoreSorted$PTM_FinalScore <- PTMscoreSorted$PTM_score*PTMscoreSorted$Score
  
MaxSet <- aggregate(PTM_FinalScore ~ Peptidoform+PROTEIN_LOC, data = PTMscoreSorted, max)


total <- merge(PTMscore,MaxSet,by="Peptidoform")


PformTable <- total %>% dplyr::arrange(Peptidoform) %>% distinct(Peptidoform, .keep_all = TRUE) %>% ungroup()
