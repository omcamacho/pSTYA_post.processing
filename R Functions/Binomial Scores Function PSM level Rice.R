...
# Binomial adjustment function for Rice data sets TPP pipeline #
################################################################

binAdjustRic <- function(dataf){
  PTMscore <- dataf
  PTMscore$Peptidoform <- paste0(PTMscore$Peptide_mod,"_",PTMscore$PTM.positions)
  PTMscore$Amino <- str_sub(substr(PTMscore$Peptide,1,PTMscore$PTM.positions),-1)
  head(PTMscore)
  ...
  X1$Vector<-lapply(1:nrow(X1), function(i) c(is.element(Y1$col1[i,]-X1$col1[i,],PTMscore$PTM.positions[i]),is.element(Y1$col2[i,]-X1$col2[i,],
                                              ...
                                              is.element(Y1$col5[i,]-X1$col5[i,],PTMscore$PTM.positions[i]))) 
