...
binAdjustSyn <- function(dataf){
  ...
  PTM2Unique <- cbind.data.frame(PTM2$All_USI,PTM2$PROTEIN_OCC,PTM2$Peptide_mod)
  
  colnames(PTM2Unique)<-c("All_USI","PROTEIN_OCC","Peptide_mod")
  
  PTM2Unique <- PTM2Unique %>% dplyr::arrange(All_USI) %>% distinct(All_USI, .keep_all = TRUE) %>% ungroup()
  ...
  PTM3 <- merge(PTM2Unique,PTM2, by="All_USI")
