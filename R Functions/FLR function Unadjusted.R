...
ReducedTable2$Amino <- str_sub(substr(ReducedTable2$Peptide,1,ReducedTable2$PTM.positions),-1)
...
# Order by Score 3#
ReducedTable2 <- ReducedTable2[order(-ReducedTable2$PTM.positions),]

ReducedTable2sorted3 <- ReducedTable2[order(-ReducedTable2$Unadjusted_Score),]
...
ReducedTable2sorted3 <- transform(ReducedTable2sorted3, FLR_Unadjusted = (ratio*cums2)/(Rnumber-cums2))
